function [hbsf,hbf,hf,bf,pf,hs,h,p] = frame_selection_skip_step6(fname,s,sk,n,t,b1,d,ext,a,offset)
...
M = mmread_silent(vname,(j0-j2+1+offset):(j0+offset),[],false,true);
waitbar(1);
close(hh);
...
while j > 0
    ...
    if size(M.frames,2) == 0
        j = 0;
    else
        C1 = M.frames(j3).cdata;
...
for i = 1:tp
    ...
    M = mmread_silent(vname,k+offset,[],false,true);
    for j = 1:size(M.frames,2)
        C2 = M.frames(j).cdata;
