function [R,R4,R1,R2,I5,T1,I3,Z1c,Z2c,Z0c,ar,ccr,fs,f,ft2,f1,cc2] = registration_phase_full_fast5(I1,I2,H,k,sx,sy,ds,ds2,md,s,t)
...
[R1 xdata ydata] = imtransform(I5, T1,'FillValues', max(max(I5)),...
    'XData', [1 size(I1,2)],'YData', [1 size(I1,1)]);

[R2 xdata ydata] = imtransform(I5, T2,'FillValues', max(max(I5)),...
    'XData', [1 size(I1,2)],'YData', [1 size(I1,1)]);

df(1) = sum(sum(abs(I1 - R1)));
df(2) = sum(sum(abs(I1 - R2)));
df(3) = sum(sum(abs(I1 - I5)));

[si,sj] = min(df);

if sj == 1
    R = R1;
    R4 = imtransform(H, T1,'FillValues', 0,...
