function [Mm,Mgb,M,Mg,Mc,R,R4,D,C,H,L,G,F0,t_ff,t_fm] = mosaic_images(dname,F0,bi,d)
...
for i = 1:lf0
    ...
    if i == 1
        P = normalize_image(padarray(L{i},[351,351],mode(mode(L{i}))),1);
    end
...
for i = 1:ll
    [R{i},R4{i}] = ...
        register_images(normalize_image(L{1},1),normalize_image(L{i+1},1),...
        H{i+1},30,20,20,5,2,15,0.25,0.03);
    waitbar(i/ll);
end
close(hh);

% STEP 3: FUSE THE IMAGES
Mg = normalize_image(P,1);
Mc = double(Hp{1});
a = -0.025;
...
lr = length(R);
for i = 1:lr
    Mg = normalize_image(min(Mg,R{i}),1);
...
% STEP 4: CREATE THE FINAL MOSAIC
p = 20;
...
Mgb = uint8(normalize_image(gabor_only2(Mg,opt)));

M = vessel_enhancement(Mc,Mg,0.85);
Mm = mosaic_mix(M,Mgb,0.75);

% WRITE THE MOSAIC IMAGES
