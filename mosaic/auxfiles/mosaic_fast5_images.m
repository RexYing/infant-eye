function [Mm,Mgb,M,Mg,Mc,R,R4,H,L,G,F0,t_ff,t_fm] = mosaic_fast5_images(dname,V,nums)

if nargin == 1
    nums = [1,2,3,4,5];
    opt = options_defaults;
    bi = opt.b;
end

opt = option_defaults;
% opt.b = bi;
% opt.d = d;

tic;

F0 = cell(length(nums),1);
for i = 1:length(nums)
    F0{i} = V{nums(i)};
end

pd = [round(size(F0{1},1)/3),round(size(F0{1},2)/3)];
H = F0; L = F0; G = F0;
hh = waitbar(0,'Filtering frames: step 1 of 4');
lf0 = length(F0);
for i = 1:lf0
    [H{i},L{i}] = filter_frame_extend2(F0{i},opt,[5,5]);
    if i == 1
        P = normalize1(padarray(L{i},pd,mode(mode(L{i}))),1);
    end
    Hp{i} = padarray(H{i},pd,0);
    waitbar(i/lf0);
end
close(hh)
t_ff = toc;

tic;
hh = waitbar(0,'Registering frames: step 2 of 4');
ll = numel(L)-1;
R = cell(numel(L),1);
R4 = R;
R{1} = P;
R4{1} = Hp{1};
for i = 2:numel(L)
    [R0,R4{i}] = ...
        registration_phase_full_fast7(normalize1(R{i-1},1),normalize1(L{1},1),normalize1(L{i},1),...
        H{i},200,25,25,5,2,15,0.25,0.03,pd);    
    R{i} = normalize1(min(R{i-1},R0),1);
    waitbar((i-1)/ll);
end
close(hh);

Mg = normalize1(P,1);
Mc = double(Hp{1});
a = -0.025;
Mc = feather_single(Mc,a);
hh = waitbar(0,'Fusing frames: step 3 of 4');
lr = length(R);
for i = 2:lr
    Mg = normalize1(min(Mg,R{i}),1);
    Mc2 = feather_single(double(R4{i}),a);
    Mc = max(Mc,Mc2);
    waitbar(i/lr);
end
close(hh);

% display_image(uint8(Mc),1);
% drawnow;

% Trim borders
p = 20;
B = bwlabel(max(Mc > 0,[],3));
[a,val] = hist(B(:),0:max(B(:)));
a(1) = [];
val(1) = [];
mv = val(a==max(a));
B(B ~= mv) = 0;
B = B > 0;
% display_image(B,1);
% drawnow;

% Mg(B == 0) = 1;
% for i = 1:size(Mc,3)
%     Mc0 = Mc(:,:,i);
%     Mc0(B == 0) = 0;
%     Mc(:,:,i) = Mc0;
% end

b0 = regionprops(B,'BoundingBox');
b02 = b0.BoundingBox;
% b02 = b0(1).BoundingBox;
% for i = 2:length(b0)
%     b03 = b0(i).BoundingBox;
%     b02(1) = max(1,min(b02(1),b03(1)));
%     b02(2) = max(1,min(b02(2),b03(2)));
%     b02(3) = max(b02(3),b03(3));
%     b02(4) = max(b02(4),b03(4));
% end
xn = floor(b02(2));
xx = ceil(b02(2)+b02(4));
yn = floor(b02(1));
yx = ceil(b02(1)+b02(3));

Mc = Mc(xn:xx,yn:yx,:);
Mc = padarray(Mc,[p,p],0);
Mg = Mg(xn:xx,yn:yx,:);
Mg = padarray(Mg,[p,p],1);

B = B(xn:xx,yn:yx,:);
B = padarray(B,[p,p],0);

opt = option_defaults2;
Mgb = uint8(normalize1(gabor_only2(Mg,opt)));

M = vessel_enhancement(Mc,Mg,0.85);
Mm = mosaic_mix(M,Mgb,0.75);
for i = 1:size(Mm,3)
    M0 = Mm(:,:,i);
    M0(B == 0) = 0;
    Mm(:,:,i) = M0;
end

imwrite(Mm,strcat(dname,'mosaic_mix_075.png'));
imwrite(uint8(Mc),strcat(dname,'mosaic_color_only.png'));
imwrite(M,strcat(dname,'mosaic_color_enhanced.png'));
imwrite(Mgb,strcat(dname,'mosaic_gabor.png'));
t_fm = toc;
