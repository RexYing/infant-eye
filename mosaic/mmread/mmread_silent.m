function [video, audio] = mmread_silent(filename, frames, time, disableVideo, disableAudio, matlabCommand, trySeeking, useFFGRAB)
...
%             hh = waitbar(0,'Determining best frames: step 1 of 2');
...
%             close(hh);
        ...
        try
            ...
            if ~isempty(strfind(err.message,'libavbin'))
                switch mexext
                    case 'mexa64'
                        if ~isempty(strfind(err.message,'No such file')) | ~isempty(strfind(err.message,'ELFCLASS32'))
                    ...
                    case 'mexglx'
                        if ~isempty(strfind(err.message,'No such file')) | ~isempty(strfind(err.message,'ELFCLASS64'))
