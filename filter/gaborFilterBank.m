function gaborArray = gaborFilterBank(u0, u,v,m,n)
...
if (nargin ~= 5)    % Check correct number of arguments
    error('There should be five inputs.')
end
...
for i = 1:u-u0+1
    for j = 1:v        
        subplot(u - u0+1,v,(i-1)*v+j);        
        imshow(abs(gaborArray{i+u0-1,j}),[]);
    end
end
...
for i = 1:u-u0+1
    for j = 1:v        
        subplot(u-u0+1,v,(i-1)*v+j);        
        imshow(real(gaborArray{i+u0-1,j}),[]);
    end
end
